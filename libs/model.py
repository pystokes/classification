...
class CNN(nn.Module): # nn.Moduleを継承する
    ...
    # Forward計算の定義
    # 参考：Define by Runの特徴（処理を動的に変更できる）
    def forward(self, x, mode='train'):
        ...
        # 直前のblock2からの出力が2次元（× チャネル数）なので，全結合（1次元）の入力形式に変換
        #
        # 変換前：[バッチ数, チャネル数, 高さ, 幅] (4次元)
        # 変換後：[バッチ数, チャネル数 x 高さ x 幅] (2次元)
        #
        # view()：引数で指定したサイズに変換する
        #   x.size(0): バッチサイズ
        #          -1: -1を指定することで，「残りの次元（C, H, W）をすべてまとめて1次元にする」の意味になる
        #
        # 参考：KerasのFlatten()と同じような処理
        x = x.view(x.size(0), -1)
 
        # ここでの出力は確率になっていないことに注意
